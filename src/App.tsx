import { useEffect, useState } from 'react'; // Se quitó 'React'
import './App.css'; 
import { db, auth } from './firebase'; 
import { collection, getDocs, doc, writeBatch } from 'firebase/firestore'; // Se quitó updateDoc
...
interface UsuarioData extends DocumentData { uid: string; email: string | null; rol: 'admin' | 'delegado' | 'pendiente' | 'jugador'; equipoId?: string; }
...
function App() {
  ...
  const generateCalendar = async () => {
    if (equipos.length < 2) { alert("Error: Necesitas al menos 2 equipos."); return; }
    setGeneratingCalendar(true);
    const calendarioRef = collection(db, 'calendario');
    const currentCalendarCheck = await getDocs(calendarioRef);
    if (currentCalendarCheck.docs.length > 0 && !window.confirm(`¿Borrar y regenerar calendario?`)) { setGeneratingCalendar(false); return; }
    
    const equiposIds = equipos.map(e => ({ id: e.id, nombre: e.nombre }));
    let n = equiposIds.length; let teams = [...equiposIds];
    if (n % 2 !== 0) { teams.push({ id: 'BYE', nombre: 'DESCANSO' }); n += 1; }
    const rounds = n - 1; const matchesPerRound = n / 2; const allMatches = [];
    for (let round = 0; round < rounds; round++) {
      for (let i = 0; i < matchesPerRound; i++) {
        const team1 = teams[i]; const team2 = teams[n - 1 - i];
        if (team1.id !== 'BYE' && team2.id !== 'BYE') {
          const isReversed = round % 2 === 1; 
          allMatches.push({ jornada: round + 1, equipoLocalId: isReversed ? team2.id : team1.id, equipoVisitanteId: isReversed ? team1.id : team2.id, equipoLocalNombre: isReversed ? team2.nombre : team1.nombre, equipoVisitanteNombre: isReversed ? team1.nombre : team2.nombre, fechaAsignada: null, partidoRegistradoId: null });
        }
      }
      teams = [teams[0], ...teams.slice(1)]; teams.splice(1, 0, teams.pop()!);
    }
    try {
      const batch = writeBatch(db);
      currentCalendarCheck.docs.forEach(doc => batch.delete(doc.ref));
      allMatches.forEach(match => batch.set(doc(collection(db,'calendario')), match));
      await batch.commit(); alert(`✅ Calendario generado.`); refreshData();
    } catch (e) { alert('Error.'); } finally { setGeneratingCalendar(false); }
  };
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      if (u) {
        ...
        setUser(d.exists() ? { uid: u.uid, email: u.email, rol: d.data().rol||'pendiente', equipoId: d.data().equipoId } : { uid: u.uid, email: u.email, rol: 'pendiente' });
      } else setUser(null);
      setLoading(false);
    });
    return () => unsub();
  ...
  if (user.rol === 'pendiente') return <div className="login-wrapper"><div className="login-box"><h2>⏳ Pendiente</h2><p>Esperando aprobación.</p><button onClick={()=>signOut(auth)} className="btn btn-secondary">Salir</button></div></div>;

  const currentEquipoName = formas21.find(f => f.id === selectedFormId || f.id === viewRosterId)?.nombreEquipo || 'Equipo';
  const isDashboard = !(viewRosterId || matchView || adminFormView || usersView || registroView || selectedFormId || calendarView || mesaTecnicaView || statsView || standingsView || selectForma5MatchId || liveMatchId);
  ...
  return (
    <div className="app-container">
      ...
      <main className="main-content">
        ...
        {calendarView && <CalendarViewer rol={user.rol} userEquipoId={user.equipoId || null} onClose={() => setCalendarView(false)} onViewLive={(id) => { setCalendarView(false); setLiveMatchId(id); }} />}
        ...
        {isDashboard && (
            <div className="animate-fade-in">
                <div className="card" style={{borderLeft:'5px solid var(--primary)', padding:'1.5rem', marginBottom:'25px'}}>
                    <h2 style={{marginTop:0, fontSize:'1.5rem', color:'var(--primary)'}}>Hola, {user.rol.toUpperCase()}</h2>
                    <p style={{color:'var(--text-muted)', marginBottom:0}}>Bienvenido al panel de control.</p>
                </div>
                ...
                <div className="dashboard-grid">
                    ...
                    <div className="dashboard-card" onClick={()=>setStatsView(true)}><div className="card-icon">📊</div><div className="card-title">Estadísticas</div></div>
                </div>
                ...
                {user.rol === 'admin' && (
                    <div style={{marginTop:'30px'}}>
                        ...
                        <div className="dashboard-grid">
                            ...
                            <div className="dashboard-card" onClick={generateCalendar} style={{background:generatingCalendar?'#f3f4f6':'white'}}><div className="card-icon">⚙️</div><div className="card-title">{generatingCalendar?'...':'Generar Cal.'}</div></div>
                        </div>
                    </div>
                )}
            </div>
